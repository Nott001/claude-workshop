...
describe("readInvitedRole", () => {
  ...
  it("refuses super_admin, which no invite may grant", () => {
    ...
    expect(readInvitedRole({ [INVITED_ROLE_KEY]: "super_admin" })).toBeNull();
  });

  it("refuses attendee so the caller's own default is used", () => {
    expect(readInvitedRole({ [INVITED_ROLE_KEY]: "attendee" })).toBeNull();
  });

  it.each([undefined, null, "admin", 42, [], {}, { role: "admin" }, { [INVITED_ROLE_KEY]: null }])(
    "returns null for %j",
    (metadata) => {
  ...
  it("ignores an unrelated key that merely looks like a role", () => {
    expect(readInvitedRole({ user_role: "admin", provider: "email" })).toBeNull();
  });
