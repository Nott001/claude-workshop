...
describe("Course room access — speaker ticket bypass", () => {
  it("allows an assigned speaker holding no ticket", () => {
    expect(canAccessCourseRoom(ROLES.SPEAKER, { hasTicket: false, isSpeakerAssigned: true })).toBe("allowed");
  });

  it("denies an unassigned speaker", () => {
    expect(canAccessCourseRoom(ROLES.SPEAKER, { hasTicket: false, isSpeakerAssigned: false })).toBe("denied");
  });

  it("allows an assigned speaker who also holds a ticket", () => {
    expect(canAccessCourseRoom(ROLES.SPEAKER, { hasTicket: true, isSpeakerAssigned: true })).toBe("allowed");
  });

  it("denies an unassigned speaker even with a ticket — assignment gates the room", () => {
    expect(canAccessCourseRoom(ROLES.SPEAKER, { hasTicket: true, isSpeakerAssigned: false })).toBe("denied");
  });
});

describe("Course room access — everyone else", () => {
  it("allows an attendee with a valid ticket", () => {
    expect(canAccessCourseRoom(ROLES.ATTENDEE, { hasTicket: true, isSpeakerAssigned: false })).toBe("allowed");
  });

  it("turns away an attendee without a ticket", () => {
    expect(canAccessCourseRoom(ROLES.ATTENDEE, { hasTicket: false, isSpeakerAssigned: false })).toBe("no_ticket");
  });

  it("lets a facilitator in on role alone", () => {
    expect(canAccessCourseRoom(ROLES.FACILITATOR, { hasTicket: false, isSpeakerAssigned: false })).toBe("allowed");
  });

  it("lets an admin in on role alone", () => {
    expect(canAccessCourseRoom(ROLES.ADMIN, { hasTicket: false, isSpeakerAssigned: false })).toBe("allowed");
  });
