...
describe("StaffUsersPage", () => {
  ...
  it("types in search, fires a fetch with search= after the debounce", async () => {
    ...
    expect(urls[urls.length - 1]).toBe("/api/users?page=1&limit=10");
    ...
    expect(urls[urls.length - 1]).toBe("/api/users?page=1&limit=10&search=ada");
  });
  ...
  it("filters by role via the select", async () => {
    ...
    expect(fetchMock).toHaveBeenLastCalledWith("/api/users?page=1&limit=10&role=facilitator");
  });

  it("shows Staff only by default and widens to every role when All users is chosen", async () => {
    stubFetch([], 0);
    ...
    await act(async () => {
      fireEvent.click(screen.getByRole("combobox"));
      const allUsers = await screen.findByRole("option", { name: "All users" });
      fireEvent.pointerDown(allUsers, { pointerType: "mouse" });
      fireEvent.click(allUsers);
    });

    await waitFor(() => expect(fetchMock).toHaveBeenLastCalledWith("/api/users?page=1&limit=10&scope=all"));
  });
  ...
  it("paginates with page=2", async () => {
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenLastCalledWith("/api/users?page=2&limit=10"));
  });
  ...
  it("deletes a member through the confirmation dialog, then refreshes", async () => {
    stubFetch([MEMBER, FACILITATOR], 2);
    ...
    expect(await screen.findByText(/will be permanently deleted/)).toBeTruthy();
    fireEvent.click(screen.getByRole("button", { name: "Delete" }));

    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith("/api/users/2", { method: "DELETE" }));
    await waitFor(() => expect(fetchMock).toHaveBeenLastCalledWith("/api/users?page=1&limit=10"));
    expect(await screen.findByText("Ben Facilitator was deleted.")).toBeTruthy();
  });

  it("shows a failed delete in the dialog and still refetches the roster", async () => {
    fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      if (init?.method === "DELETE") {
        return {
          ok: false,
          status: 500,
          json: async () => ({ error: "Failed to delete the auth account: User not found" }),
        };
      }
      return { ok: true, json: async () => ({ users: [MEMBER, FACILITATOR], total: 2 }) };
    });
    vi.stubGlobal("fetch", fetchMock);
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith("/api/users/2", { method: "DELETE" }));
    expect(await screen.findByText(/Failed to delete the auth account: User not found/)).toBeTruthy();
    await waitFor(() => expect(fetchMock).toHaveBeenLastCalledWith("/api/users?page=1&limit=10"));
  });

  it("hides the delete action for the signed-in user's own row", async () => {
    ...
    expect(screen.getByRole("button", { name: "Delete user" }).hasAttribute("disabled")).toBe(true);
  });
  ...
  it("filters to attendees so one can be found to promote", async () => {
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenLastCalledWith("/api/users?page=1&limit=10&role=attendee"));
  });
  ...
  it("explains and disables the delete button for a peer admin", async () => {
    ...
    expect(screen.getByText("You can only delete users in a role you outrank.")).toBeTruthy();
    expect(screen.getByRole("button", { name: "Delete user" }).hasAttribute("disabled")).toBe(true);
  });
