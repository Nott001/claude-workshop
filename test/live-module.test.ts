...
import { findLiveModule, type LiveModuleSource } from "@/modules/events/lib/live-module";
...
function at(hour: number, minute: number, second = 0): Date {
  return new Date(
    `${EVENT_DATE}T${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}:${String(second).padStart(2, "0")}`,
  );
}
