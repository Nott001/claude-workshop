...
const { requireRole, findRoleById, updateRole, removeById, logAuditEvent } = vi.hoisted(() => ({
  requireRole: vi.fn(),
  findRoleById: vi.fn(),
  updateRole: vi.fn(),
  removeById: vi.fn(),
...
vi.mock("@/shared/db/dao/user.dao", () => ({ findRoleById, updateRole, removeById }));
vi.mock("@/modules/audit/lib/log-audit-event", () => ({
  logAuditEvent,
  requireAuditEvent: vi.fn(async (...args: unknown[]) => logAuditEvent(...args)),
}));

import { PATCH, DELETE } from "@/app/api/organization/[userId]/route";
...
function patch(userId: string, body: unknown) {
  return new Request(`https://app.test/api/organization/${userId}`, { method: "PATCH", body: JSON.stringify(body) });
}

function del(userId: string) {
  return new Request(`https://app.test/api/organization/${userId}`, { method: "DELETE" });
}
...
describe("PATCH /api/organization/[userId]", () => {
  ...
  it("records both roles in the audit trail", async () => {
    ...
    expect(logAuditEvent).toHaveBeenCalledWith(expect.anything(), 3, "organization.role_changed", "user", 9, {
...
describe("DELETE /api/organization/[userId]", () => {
  it("removes an ordinary member", async () => {
    const res = await DELETE(del("9"), params("9"));

    expect(res.status).toBe(200);
    expect(removeById).toHaveBeenCalledWith(expect.anything(), 9);
  });
