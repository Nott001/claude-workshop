...
Brevo service must be replaced by Resend. Currently, Brevo has not been tested nor has it been confirmed working. Use resend as a replacement for Brevo. Eamils
will send key lifecycle events, most especially reminders and ticket issuance must be logged. This isn't used for logging, more so as a convenience as tickets
can already be found in the application.

## Scope

- Tables in supabase already exist, check the directory `supabase/migrations` for the related script.
- Remove all code that was implemented by brevo which will not be used.
- Add another section in the facilitator navbar to view all emails, implement this page as well.
- Ask me questions to clarify actions and instructions before committing each change

## References
From the Resend prompt
