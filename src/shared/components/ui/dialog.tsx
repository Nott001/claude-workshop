...
function DialogOverlay({ className, ...props }: DialogPrimitive.Backdrop.Props) {
  return <DialogPrimitive.Backdrop className={cn("fixed inset-0 z-50 bg-overlay", className)} {...props} />;
}
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        className={cn(
          "fixed top-1/2 left-1/2 z-50 w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl bg-surface p-4 text-sm text-surface-fg shadow-lg outline-none sm:max-w-sm",
          className,
        )}
        {...props}
      >
...
function DialogDescription({ className, ...props }: DialogPrimitive.Description.Props) {
  return <DialogPrimitive.Description className={cn("text-sm text-muted-fg", className)} {...props} />;
}
