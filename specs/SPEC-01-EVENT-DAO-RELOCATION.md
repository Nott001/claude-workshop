...
- Update the `event.dao` import site in each of the 13 consumers:
  ...
  - `src/app/api/speakers/me/events/route.ts` and `[id]/route.ts`
...
- Update the ~10 test files that import `event.dao` (specs/registration, event-room
  variants, events CRUD, api handlers) to the new path.
...
- 13 import sites + ~10 test files (path updates only)
